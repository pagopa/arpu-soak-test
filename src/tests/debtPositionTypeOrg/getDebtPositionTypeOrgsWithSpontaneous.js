...
const testName = "getDebtPositionTypeOrgsWithSpontaneous";
...
export const options = defaultApiOptionsBuilder(
  ...
  testName,
  [
    DEBT_POSITION_TYPE_ORG_API_NAMES.getDebtPositionTypeOrgsWithSpontaneous,
...
export default (data) => {
  ...
  const getDebtPositionTypeOrgsWithSpontaneousResult = getDebtPositionTypeOrgsWithSpontaneous(data.brokerId, organizationId, data.token);

  assert(getDebtPositionTypeOrgsWithSpontaneousResult, [statusOk()]);

  if (getDebtPositionTypeOrgsWithSpontaneousResult.status !== 200) {
    logErrorResult(
      `Unexpected getDebtPositionTypeOrgsWithSpontaneousResult status`,
